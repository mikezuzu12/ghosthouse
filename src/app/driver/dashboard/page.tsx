...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border ${s.color}`}
    >
      <span
...
export default function DriverDashboard() {
  ...
  const [tab, setTab] = useState<"available" | "claimed" | "delivered">(
    "available"
  );
  ...
  useEffect(() => {
    ...
    const fetchDriverLocation = async () => {
      try {
        const res = await fetch(`/api/drivers/location?order_id=${showTracking}`);
  ...
  async function claimOrder(id: string) {
    setClaiming(id);
    try {
      const res = await fetch(`/api/drivers/orders/${id}/claim`, {
        method: "POST",
      });
  ...
  async function completeOrder(id: string) {
    setCompleting(id);
    try {
      const res = await fetch(`/api/drivers/orders/${id}/complete`, {
        method: "POST",
      });
  ...
  return (
    <div
      ...
      style={{ background: "#0A0E1A", color: "#F0F4FF" }}
    >
      <AnimatePresence>
        {notifications.map((n) => (
          <motion.div
            ...
            className={`fixed top-20 right-4 z-50 px-5 py-3 rounded-xl shadow-2xl border text-sm font-medium min-w-[250px] backdrop-blur-xl ${
              n.type === "success"
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <AnimatePresence>
              {filteredOrders.map((order, index) => (
                ...
                >
                  <div
                    className="px-5 py-4 border-b border-white/8 flex items-start justify-between"
                    style={{
                      background:
                        "linear-gradient(90deg, rgba(59,130,246,0.08) 0%, rgba(6,182,212,0.04) 100%)",
                    }}
                  >
                  ...
                  <div className="px-5 py-4 space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-white/8">
                      ...
                      {tab === "claimed" && (
                        <button
                          onClick={() =>
                            setShowTracking(
                              showTracking === order.id ? null : order.id
                            )
                          }
                    ...
                    {tab === "delivered" && (
                      <div
                        className="w-full py-2.5 rounded-xl text-center font-semibold flex items-center justify-center gap-2 text-sm"
                        style={{
              ...
              <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                <div>
                  ...
                  >
                    {[
                      ...
                      ["Email", selectedOrder.customer_email || "Not provided"],
                    ].map(([label, value]) => (
                ...
                <div>
                  ...
                  >
                    ...
                    <div className="border-t border-white/8 pt-2 mt-2 flex justify-between font-bold">
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">
                      #{showTracking.slice(0, 8).toUpperCase()}
                    </p>
                  </div>
                  <button
                    onClick={() => setShowTracking(null)}
                ...
                {loadingLocation ? (
                  ...
                  >
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-slate-300">
                        {driverLocation.accuracy?.toFixed(0) || "N/A"} meters
                      </span>
                    </div>
                  </div>
                ) : (
                  ...
                  >
                    <p className="text-slate-400 text-sm">
                      ...
                      above to begin sharing your location with the customer.
                    </p>
                  </div>
                )}
              </div>
